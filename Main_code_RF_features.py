...
def aggregate_trip_data(df):
    ...
    cov_speed_acc = df.groupby(['User', 'Trip']).apply(lambda g: calc_covariance(g['Speed'], g['Acc'])).reset_index(
        drop=True)
    cov_acc_speed = df.groupby(['User', 'Trip']).apply(lambda g: calc_covariance(g['Acc'], g['Speed'])).reset_index(
        drop=True)
    ...
    hvr, mvr, lvr = zip(
        *df.groupby(['User', 'Trip'])['Speed'].apply(lambda x: rate_category(x, [10, 30])).apply(pd.Series).values)
    har, mar, lar = zip(
        *df.groupby(['User', 'Trip'])['Acc'].apply(lambda x: rate_category(x, [0.1, 0.5])).apply(pd.Series).values)
...
# Aggregate the combined dataframe
aggregated_df = aggregate_trip_data(combined_df)

# Check if 'Mode' column exists
if 'Mode_first' not in aggregated_df.columns:
    raise ValueError("'Mode' column not found in the DataFrame")
...
mode_categories = train_df['Mode_first'].astype('category').cat.categories
train_df['Mode_first'] = train_df['Mode_first'].astype('category').cat.codes
train_features = train_df.drop(['Mode_first', 'User', 'Trip'], axis=1, errors='ignore')
train_classes = train_df['Mode_first']

# Preprocess the testing data
test_df.dropna(inplace=True)
test_df['Mode_first'] = test_df['Mode_first'].astype('category').cat.codes
test_features = test_df.drop(['Mode_first', 'User', 'Trip'], axis=1, errors='ignore')
test_classes = test_df['Mode_first']
