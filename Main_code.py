...
# Function to aggregate trip data
...
# Check if 'Mode' column exists
if 'Mode' not in aggregated_df.columns:
    raise ValueError("'Mode' column not found in the DataFrame")
...
mode_categories = train_df['Mode'].astype('category').cat.categories
train_df['Mode'] = train_df['Mode'].astype('category').cat.codes
train_features = train_df.drop(['Mode', 'User', 'Trip'], axis=1, errors='ignore')
train_classes = train_df['Mode']

# Preprocess the testing data
test_df.dropna(inplace=True)
test_df['Mode'] = test_df['Mode'].astype('category').cat.codes
test_features = test_df.drop(['Mode', 'User', 'Trip'], axis=1, errors='ignore')
