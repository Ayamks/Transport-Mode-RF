...
with open('004_Preprocessed_Train_Data.pickle', 'rb') as f:
    train_df = pickle.load(f)

# Load preprocessed testing data from pickle file
with open('004_Preprocessed_Test_Data.pickle', 'rb') as f:
    test_df = pickle.load(f)

# Check if 'Mode' column exists
if 'Mode' not in train_df.columns or 'Mode' not in test_df.columns:
...
# Check for overlapping trips between training and test datasets
train_trips = set(train_df['Trip'].unique())
test_trips = set(test_df['Trip'].unique())
overlapping_trips = train_trips.intersection(test_trips)

# Print results and raise an error if there are overlapping trips
if overlapping_trips:
    raise ValueError(f"Trips present in both training and test sets: {overlapping_trips}")
else:
    print("No trips are shared between the training and test sets.")
